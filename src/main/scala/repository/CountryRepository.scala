...
class CountryRepository(transactor: Transactor[IO]) {
  ...
  def getCountries: Stream[IO, Country] =
    sql"""select code, name, continent, region, surfacearea, population, localname, governmentform, code2 
          from country 
  ...
  def getCountry(code: String): IO[Either[CountryNotFoundError, Country]] =
    sql"""select code, name, continent, region, surfacearea, population, localname, governmentform, code2 
          from country 
          where code = $code"""
      .query[Country]
      ...
      .map {
        ...
        case _           => Left(CountryNotFoundError(code))
      }
  ...
  def createCountry(c: Country): IO[String] =
    sql"""insert into country (code, name, continent, region, surfacearea, population, localname, governmentform, code2)
          values (${c.code}, ${c.name}, ${c.continent}, ${c.region}, ${c.surfacearea}, ${c.population}, ${c.localname}, ${c.governmentform}, ${c.code2})""".update
      .withUniqueGeneratedKeys[String]("code")
      .transact(transactor)

  def deleteCountry(code: String): IO[Either[CountryNotFoundError, Unit]] =
    sql"""delete from country
          where code = $code""".update.run.transact(transactor).map { affectedRows =>
      if (affectedRows == 1)
        Right(())
      else
        Left(CountryNotFoundError(code))
    }
